<template>
  <div class="p-4">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
      <div>
        <h2 class="h4 mb-1">退款审核</h2>
        <p class="text-muted small mb-0">查看、审核用户的退款申请</p>
      </div>
      <div class="d-flex gap-2 flex-wrap">
        ...
        <el-button @click="fetchRefunds" :icon="RefreshRight">刷新</el-button>
      </div>
    </div>
    ...
    <el-card shadow="never" class="border">
      <el-table :data="refunds" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="跑腿员" width="120">
          <template #default="{ row }">
            <div>
              <div>{{ row.runner?.nickname || '-' }}</div>
              <div class="text-muted small">{{ row.runner?.student_id || '' }}</div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="amount" label="退款金额" width="120" align="center">
          <template #default="{ row }">
            <span class="fw-semibold text-warning">¥ {{ formatMoney(row.amount) }}</span>
          </template>
        </el-table-column>
...
async function fetchRefunds() {
  ...
  try {
    ...
    const res = await http.get('/admin/refund/list', { params })
    const data = res.data
    
    refunds.value = data.items || data.list || []
    pagination.value.total = data.total || 0
  } catch (err: any) {
    ElMessage.error(err?.response?.data?.message || '加载退款申请失败')
  } finally {
    loading.value = false
  }
}
...
async function handleApprove(row: Refund) {
  try {
    ...
    await http.post(`/admin/refund/${row.id}/audit`, {
      decision: 'APPROVE'
    })
...
async function confirmReject() {
  ...
  try {
    await http.post(`/admin/refund/${currentRefund.value.id}/audit`, {
      decision: 'REJECT',
